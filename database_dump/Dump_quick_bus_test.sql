CREATE DATABASE  IF NOT EXISTS `quick_bus_test` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `quick_bus_test`;
-- MySQL dump 10.13  Distrib 5.6.13, for Win32 (x86)
--
-- Host: 127.0.0.1    Database: quick_bus_test
-- ------------------------------------------------------
...
from quick_bus_test.latest_bus_position
where Vehicle_ID = new.Vehicle_ID;

select Route_Name
into var_route_name
from quick_bus_test.bus_vehicles
where Vehicle_ID = new.Vehicle_ID;
...
if (var_old_latitude is not null) and (var_old_longitude is not null) then
    if var_old_direction <> '环线' then
        ...
        from quick_bus_test.bus_route_detail
        where (abs(var_old_latitude - Start_Station_Latitude) + abs(var_old_longitude - Start_Station_Longitude)) =
            ...
            from quick_bus_test.bus_route_detail where Route_Name = var_route_name
            )
        limit 1;
    ...
    from quick_bus_test.bus_routes
...
delete from quick_bus_test.latest_bus_checkpoints
where Vehicle_ID = new.Vehicle_ID;

/* Insert check points */
insert into quick_bus_test.latest_bus_checkpoints
select *, new.Vehicle_ID as Vehicle_ID, new.report_time as report_time
from quick_bus_test.bus_route_station_detail
where abs(new.Latitude - Checkpoint_Latitude) + abs(new.Longitude - Checkpoint_Longitude) < var_in_checkpoint_distance
    ...
    and Direction = var_new_direction and Route_Name = (select Route_Name from quick_bus_test.bus_vehicles where Vehicle_ID = new.Vehicle_ID);
...
where abs(new.Latitude - Latitude) + abs(new.Longitude - Longitude) =
    ...
    from quick_bus_test.bus_route_station_detail
    where Direction = var_new_direction and Route_Name = (select Route_Name from quick_bus_test.bus_vehicles where Vehicle_ID = new.Vehicle_ID)
    )
limit 1;
...
delete from quick_bus_test.latest_bus_position
where Vehicle_ID = new.Vehicle_ID;

INSERT INTO quick_bus_test.latest_bus_position
...
INSERT INTO quick_bus_test.debug_latest_bus_checkpoints
...
if new.is_in_station = 1 then
    INSERT INTO quick_bus_test.debug_latest_bus_position
...
INSERT INTO `user_route_points` VALUES (1,1,31.249903,121.513914),(1,2,31.257559,121.544959),(1,3,31.256571,121.593540),(1,4,31.296569,121.571118);
...
INSERT INTO `user_routes` VALUES (1,6,'r1asdf');
...
--
-- Dumping events for database 'quick_bus_test'
--

--
-- Dumping routines for database 'quick_bus_test'
--
...
-- Dump completed on 2014-03-01 19:22:27
